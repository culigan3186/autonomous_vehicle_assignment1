...
class YOLO:
    ...
    def make_predictions(self,  image = None, img_path= None,plot =False , save_path=None, obstructions =  None, ar_th=0.0004):
        ...
        for i in range(len(yolos)):
            ...
            # decode_netout에서는 바운딩 박스 후보와 라벨(클래스)를 예측합니다.

        correct_yolo_boxes(boxes, image_h, image_w, self.net_h, self.net_w,ar_th=ar_th)
        # correct_yolo_boxes에서는 decode_netout에서 그렸던 박스들을 이미지의 원래크기에 맞혀 바꿔주는 함수입니다.
        do_nms(boxes, self.nms_thresh)   # 여러 바운딩 박스들이 겹치기 때문에 겹치는 박스들 중 최대값을 가진 것을 제외하고 제거합니다.
        # print(len(boxes))
        if plot :
            draw_boxes(image, boxes, np.asarray(self.labels)[obstructions], self.obj_thresh)  # 박스를 이미지에 그려주는 함수입니다.
