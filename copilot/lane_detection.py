...
ORANGE =(0,165,255)
BLACK =(0,0,0)
vehicles = [1,2,3,5,6,7,8]     # class중 vehicles에 해당하는 인덱스를 나타냅니다. [1]은 bicycle [2]는 car등
animals =[15,16,17,18,19,21,22,23,]  # class 중 animals에 해당하는 인덱스를 나타냅ㄴ디ㅏ.
humans =[0] # class 중 사람에 해당하는 인덱스는 [0] 입니다. 'Ped'로 저장되어 있습니다.
...
# OBSTACLE에 해당하는 바운딩박스에 관한 함수를 class로 정의함
...
class LANE_DETECTION:
    ...
    def calc_perspective(self,  lane_start=[0.25,0.75]):
        ...
        self.lane_roi = np.zeros((self.img_dimensions[0], self.img_dimensions[1]), dtype=np.uint8)  # lane의 roi 영역을 초기화합니다.
        Lhs = np.zeros((2,2), dtype= np.float32)
        Rhs = np.zeros((2,1), dtype= np.float32)
        grey = cv2.cvtColor(self.image, cv2.COLOR_BGR2GRAY) # BGR 이미지를 GRAY 로 변환합니다. 
        mn_hsl = np.median(grey)
        edges = cv2.Canny(grey, int(mn_hsl), int(mn_hsl*.3))     # canny 함수를 적용하여 엣지를 검출합니다.
        #HoughLinesP 함수를 사용하면 선의 시작점과 끝점을 Return 해줘서 이용하기 편리합니다.
...
if __name__ == "__main__":
    ...
    nb_frames = int(video_reader.get(cv2.CAP_PROP_FRAME_COUNT))  # 동영상의 총 프레임 수를 nb_frames에 저장합니다.
    frame_h = int(video_reader.get(cv2.CAP_PROP_FRAME_HEIGHT))
    # frame의 높이를 frame_h에 저장합니다.
    frame_w = int(video_reader.get(cv2.CAP_PROP_FRAME_WIDTH))
    # frame의 너비를 frame_w에 저장합니다.
